...
//***********************SHIPPING INFORMATION********************************************
	...
	echo '<tr>';
	echo '<td> State: </td>';
	echo '<td> <select id="stt" name = "state" required> ';
			while($cRow = mysqli_fetch_array($res))
			{
				if($cRow['0']==$_POST['state'])
				echo '<option selected="selected">'.$cRow['0'].'</option>';
				else			
	...
	echo '<tr>';
	echo '<td> Zip Code </td>';
	echo '<td> <input type = "text" placeholer = "zip code" name = "zcode" value="'.$_POST['zcode'].'" required/> </td>';
	echo '</tr>';                 
	echo '<tr>';
	...
	echo '<tr/>';	
	echo '<tr>';
	//echo '<td colspan = "2" align = "center"><input type = "submit" name = "submitCustDetails" value="Submit Customer Details" align = "center"> </input> </td>';
	...
	//echo '<td> <input type="submit" name="getNormalCharges" value="Get Normal Delivery Charges"> </form> </td>';
    echo '</tr>';
...
if(isset($_POST['getCharges']))
{
	...
	include 'LocalhostConnection.php';
	$t= $_POST['state'];
	$fastDelivery="select * from fastDelivery where state='".$t."';";
	//echo $fastDelivery;
  ...
  //echo 'Hello';  
	...
	echo '<td> <center> '.$priceRow['0'].'*'.$priceRow['1'].' </center> </td>';
	echo '</tr>';
	echo '<tr>';
	...
	$updateOrder = "UPDATE orders SET price = ((".$priceRow['0']."*".$additionalChargeRow['1']."*".$priceRow['1']."/100)+(".$priceRow['0'].")+(".$additionalChargeRow['2'].")+".$additionalChargeRow['3']."+".$fast['1'].") 
	...
	//if (isset($_POST['getCharges']) and $fastDeliveryCharges==false)
		//if (isset($_POST['getCharges']))
	...
	//echo $updateOrder;
	$setPrice = mysqli_query($link,$updateOrder);
	$delCharges= "UPDATE  orders SET deliveryDays='23' WHERE orderID = ".$orderID.";";
	$dc= mysqli_query($link,$delCharges);
	$getFinalPrice = "SELECT price FROM orders WHERE orderID = ".$orderID.";";
	$FinalCharge = $link->query($getFinalPrice);
    $Charge = mysqli_fetch_array($FinalCharge);
...
if(isset($_POST['submitCustDetails']))
{
	...
	$custDetailsQuery = "INSERT INTO customer(orderID,customerName,addressLine,state,zipCode,email,phoneNumber) VALUES
						(".$orderID.",'".$_POST['Name']."','".$_POST['Address']."','".$_POST['state']."',".$_POST['zcode']."
